...
from getObsAtmo import Dict_Of_sitesAltitudes
...
def usage():
    ...
    msg = (
        ...
        "-a <airmassmin,airmassmax,nbins> "
        "-v <pwvmin, pwvmax,nbins> "
        "-o <ozmin, ozmax,nbins>"
    )
    ...
    print(
        " example : python ",
        sys.argv[0],
        " -s LSST -a 1,2.6,0.1 -v 0,15.25,0.25 -o 0.,625.0,25.",
    )
    print(
        " or      : python ",
        sys.argv[0],
        " -s LSST -a 1,2.5,16  -v 0,15.25,61  -o  0.,625.0,25 ",
    )
...
def decode_args(arg_str):
    ...
    :param arg_str:
    :type arg_str: _type_
    :raises Exception: either the input arg cannot be splint in 3 element,
    or one of element is not a number
...
def convert_dict_to_json(
    ...
    """
    Convert a Python dictionary containing NumPy arrays into a JSON-serializable
    dictionary.
    ...
    (int, float, str, bool, None) are preserved as-is.
    The resulting dictionary can be safely serialized using `json.dumps`.

    Args:
        data_dict (Dict[str, Any]): Input dictionary, possibly containing NumPy arrays
        or scalar values.
        data_json (Optional[Dict[str, Any]]): Optional output dictionary. If provided,
        the converted data will be written into it. Otherwise,
...
if __name__ == "__main__":
    ...
    msg = (
        f"airmass - grid : Npoints = {NAM}, "
        f"valmin={AIRMASSMIN:.3f}, "
        f"valmax={AIRMASSMAX:.3f}, "
        f"valstep= {DAM:.3f}"
    )
    ...
    # note we call function ProcessSimulation with proc_str='sc'
    # which mean scattering only
    ...
    for idx, am in enumerate(airmasses):
        wl, atm = libsimulateVisible.ProcessSimulation(
            am,
            pwv1,  # type: ignore[arg-type]
            oz1,  # type: ignore[arg-type]
            press_num=0,
            aer_num=0,
    ...
    # note we call function ProcessSimulation with proc_str='ab'
    # which mean absorption only
    # set pwv and ozone to zero (no need here)
    ...
    for idx, am in enumerate(airmasses):
        wl, atm = libsimulateVisible.ProcessSimulation(
            am,
            pwv2,  # type: ignore[arg-type]
            oz2,  # type: ignore[arg-type]
            press_num=0,
            aer_num=0,
            angstrom_exponent_num=1.4,
            prof_str="us",
    ...
    for idx_pwv, pwv in enumerate(pwvs):
        ...
        for idx_am, am in enumerate(airmasses):
            wl, atm = libsimulateVisible.ProcessSimulation(
                am,
                pwv,  # type: ignore[arg-type]
                oz3,  # type: ignore[arg-type]
                press_num=0,
                aer_num=0,
                angstrom_exponent_num=1.4,
                prof_str="us",
    ...
    for idx_oz, oz in enumerate(ozs):
        ...
        for idx_am, am in enumerate(airmasses):
            wl, atm = libsimulateVisible.ProcessSimulation(
                am,
                pwv4,  # type: ignore[arg-type]
                oz,  # type: ignore[arg-type]
                press_num=0,
                aer_num=0,
                angstrom_exponent_num=1.4,
                prof_str="us",
