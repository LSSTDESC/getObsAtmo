...
P0 = 101325.0  # *u.Pa;   /*!< Pa : pressure at see level */
...
def RayOptDepth_adiabatic(wavelength, altitude=altitude0, costh=1):
    ...
    A = XDepth_adiabatic(h, costh) / (3102.0 * 1e-3 / (1e-4))
    B = np.exp(-4.0 * np.log(wavelength / (400.0)))
...
def RayOptDepth_isothermal(wavelength, altitude=altitude0, costh=1):
    ...
    A = XDepth_isothermal(h, costh) / (31020.0)
    B = np.exp(-4.0 * np.log(wavelength / (400.0)))
    C = 1 - 0.0722 * np.exp(-2 * np.log(wavelength / (400.0)))
...
def RayOptDepth2_adiabatic(wavelength, altitude=altitude0, costh=1):
    ...
    A = XDepth_adiabatic(h, costh) / (27700.0)
...
def RayOptDepth2_isothermal(wavelength, altitude=altitude0, costh=1):
    ...
    A = XDepth_isothermal(h, costh) / (27700.0)
    B = np.exp(-4 * np.log(wavelength / (400)))
...
if __name__ == "__main__":
    ...
    # ---------------------------------------
    h = np.linspace(0, 20000.0, 20)
    p1 = Pressure_isothermal(h)
    p2 = Pressure_adiabatic(h)
    pmin = p2.min()
    pmax = p2.max()
    p_pdm = np.interp(altitude0, h, p2)
    print(p_pdm / 100.0, "hecto pascal")
    plt.figure()
    plt.plot(p1 / P0, h / 1000.0, "b-", label="isothermal")
    plt.plot(p2 / P0, h / 1000.0, "r-", label="adiabatic")
    plt.title("Thermodynamic model of Altitude versus Pressure")
    plt.plot([pmin / P0, pmax / P0], [altitude0 / 1000.0, altitude0 / 1000.0], "g:")
    plt.plot([p_pdm / P0, p_pdm / P0], [h.min() / 1000.0, h.max() / 1000.0], "g:")
    ...
    print(x_pdm / 100.0, "kg/m^2")

    plt.figure()
    plt.plot(XD_adiabatic, h / 1000.0, "r-", label="adiabatic")
    plt.plot(XD_isothermal, h / 1000.0, "b-", label="isothermal")

    plt.plot([xmin, xmax], [altitude0 / 1000.0, altitude0 / 1000.0], "g:")
    plt.plot([x_pdm, x_pdm], [h.min() / 1000.0, h.max() / 1000.0], "g:")
    ...
    wavelength = np.linspace(200.0, 1100.0, 100)  # in nm
